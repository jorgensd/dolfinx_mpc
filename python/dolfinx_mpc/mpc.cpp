...
{
  m.def("compute_shared_indices", &dolfinx_mpc::compute_shared_indices<T>);

  std::string pyclass_name = "MultiPointConstraint_" + type;
  // dolfinx_mpc::MultiPointConstraint
  py::class_<dolfinx_mpc::MultiPointConstraint<PetscScalar, T>,
             std::shared_ptr<dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>>
      multipointconstraint(m, pyclass_name.c_str(),
                           "Object for representing contact "
                           "(non-penetrating) conditions");
  multipointconstraint
      .def(py::init<std::shared_ptr<const dolfinx::fem::FunctionSpace<T>>,
                    std::vector<std::int32_t>, std::vector<std::int64_t>,
                    std::vector<PetscScalar>, std::vector<std::int32_t>,
                    std::vector<std::int32_t>>())
      .def_property_readonly(
          "masters",
          &dolfinx_mpc::MultiPointConstraint<PetscScalar, T>::masters)
      .def("coefficients",
           [](dolfinx_mpc::MultiPointConstraint<PetscScalar, T>& self)
           {
      ...
      .def_property_readonly(
          "owners", &dolfinx_mpc::MultiPointConstraint<PetscScalar, T>::owners)
      .def_property_readonly(
          "slaves",
          [](dolfinx_mpc::MultiPointConstraint<PetscScalar, T>& self)
          {
      ...
      .def_property_readonly(
          "cell_to_slaves",
          &dolfinx_mpc::MultiPointConstraint<PetscScalar, T>::cell_to_slaves)
      .def_property_readonly(
          "num_local_slaves",
          &dolfinx_mpc::MultiPointConstraint<PetscScalar, T>::num_local_slaves)
      .def_property_readonly(
          "function_space",
          &dolfinx_mpc::MultiPointConstraint<PetscScalar, T>::function_space)
      .def_property_readonly(
          "owners", &dolfinx_mpc::MultiPointConstraint<PetscScalar, T>::owners)
      .def(
          "backsubstitution",
  ...
  m.def("create_sparsity_pattern", &dolfinx_mpc::create_sparsity_pattern<T>);

  m.def("assemble_matrix",
        [](Mat A, const dolfinx::fem::Form<PetscScalar>& a,
           const std::shared_ptr<
               const dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>& mpc0,
           const std::shared_ptr<
               const dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>& mpc1,
  ...
  m.def(
      ...
      [](py::array_t<PetscScalar, py::array::c_style> b,
         ...
         const std::shared_ptr<
             const dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>& mpc)
      {
  ...
  m.def(
      ...
      [](const dolfinx::fem::Form<PetscScalar>& a,
         const std::shared_ptr<
             dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>& mpc)
      {
  ...
  m.def(
      ...
      [](const dolfinx::fem::Form<PetscScalar>& a,
         const std::shared_ptr<
             dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>& mpc0,
         const std::shared_ptr<
             dolfinx_mpc::MultiPointConstraint<PetscScalar, T>>& mpc1)
      {
  ...
  m.def("create_contact_slip_condition",
        &dolfinx_mpc::create_contact_slip_condition<T>);
  m.def("create_slip_condition", &dolfinx_mpc::create_slip_condition<T>);
  m.def("create_contact_inelastic_condition",
        &dolfinx_mpc::create_contact_inelastic_condition<T>);
  m.def("create_normal_approximation",
        [](std::shared_ptr<dolfinx::fem::FunctionSpace<T>> V, std::int32_t dim,
  ...
  m.def("create_periodic_constraint_geometrical",
        [](const std::shared_ptr<const dolfinx::fem::FunctionSpace<T>> V,
           const std::function<py::array_t<bool>(const py::array_t<T>&)>&
               indicator,
           const std::function<py::array_t<T>(const py::array_t<T>&)>& relation,
           const std::vector<std::shared_ptr<
               const dolfinx::fem::DirichletBC<PetscScalar>>>& bcs,
           PetscScalar scale, bool collapse)
        {
          ...
          auto _relation = [&relation](std::span<const T> x) -> std::vector<T>
          {
            assert(x.size() % 3 == 0);
            const std::array<std::size_t, 2> shape = {3, x.size() / 3};
            const py::array_t _x(shape, x.data(), py::none());
            py::array_t v = relation(_x);
            std::vector<T> output(v.data(), v.data() + v.size());
            return output;
          };
...
void mpc(py::module& m)
{
  ...
  // Template over mesh resolution
  declare_mpc<float>(m, "float32");
  // declare_mpc<double>(m, "float64");
}
} // namespace dolfinx_mpc_wrappers
