...
class MultiPointConstraint():
    ...
    def add_constraint(self, V: dolfinx.FunctionSpace, slaves: "numpy.ndarray[numpy.int32]",
                       masters: "numpy.ndarray[numpy.int64]", coeffs: "numpy.ndarray[PETSc.ScalarType]",
                       owners: "numpy.ndarray[numpy.int32]", offsets: "numpy.ndarray[numpy.int32]"):
        """
        ...
        offsets
            Array indicating the location in the masters array for the i-th slave in
            the slaves arrays.
    ...
    def create_slip_constraint(self, facet_marker: tuple([dolfinx.MeshTags, int]), v: dolfinx.Function,
        ...
        """
        Create a slip constraint :math:`dot(u, v)=0` over the entities defined in a `dolfinx.Meshtags`
    ...
    def create_contact_inelastic_condition(self, meshtags: dolfinx.MeshTags, slave_marker: int, master_marker: int):
        ...
        The interfaces should be within machine precision of eachother, but the vertices does not need to align.
        The condition created is :math:`u_s = u_m` where :math:`s` stands for the restriction to the
        slave facets, :math:`m` to the master facets.
