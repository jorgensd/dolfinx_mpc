    ...
    for i, dof in enumerate(dofs):
        ...
        for (j, line) in enumerate(operations):
            ...
            if data[0] == "Backsubstitution":
                continue
            if first:
                plt.bar(i, float(data[2]), 0.5, bottom=total_time, label=data[0], color=colors[j])
            else:
                plt.bar(i, float(data[2]), 0.5, bottom=total_time, color=colors[j])
            if data[0] == "Solve":
...
def visualize_single(dof):
    ...
    for (j, line) in enumerate(operations):
        ...
        if data[0] == "Backsubstitution":
            continue
        plt.bar(j, float(data[2]), 0.5, label=data[0], color=colors[j])
        if data[0] == "Solve":
            ax.annotate('{0:d}'.format(solve_iterations),
                        xy=(j, float(data[2]) / 2),
                        xytext=(0, 0),
    ...
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    ax.legend(bbox_to_anchor=(0.95, 0.5))
    plt.title("Average runtime of operations with {0:d} slave DOFs".format(slaves))

    plt.savefig("test_single.png")


visualize_single(dofs[0])
