    ...
    const std::shared_ptr<dolfinx_mpc::MultiPointConstraint<T, U>> mpc1)
{
  ...
  // Replace slave dofs by their masters in the sparsity pattern.
  //
                ...
                mpc_col)
  {
    ...
    const std::size_t bs_row = V_row->dofmap()->index_map_bs();
    const std::size_t bs_col = V_col->dofmap()->index_map_bs();

    // Map from cell index (local to the respective space's mesh) to the slave
    // dofs in that cell. Only cells owned by the process have entries.
    const std::shared_ptr<const dolfinx::graph::AdjacencyList<std::int32_t>>
        cell_to_row_slaves = mpc_row->cell_to_slaves();
    const std::shared_ptr<const dolfinx::graph::AdjacencyList<std::int32_t>>
        cell_to_col_slaves = mpc_col->cell_to_slaves();

    // Scratch reused across cells to avoid reallocating per entity
    std::vector<std::int32_t> col_dofs;
    std::vector<std::int32_t> master_rows;

    // Insert the master rows/columns generated by one pair of integration
    // cells. The plain (non-master) entries are already covered by
    ...
    auto insert_entity = [&](std::int32_t cell_row, std::int32_t cell_col)
    {
      ...
      col_dofs.clear();
      std::ranges::copy(V_col->dofmap()->cell_dofs(cell_col),
                        std::back_inserter(col_dofs));
      if (col_has_slaves)
      {
        // FIXME: Could remove dofs that are slaves
        expand_masters_for_slave(col_dofs, bs_col, *col_master_map,
                                 cell_to_col_slaves->links(cell_col));

        // The columns gained masters, so the ordinary rows need them too
        pattern.insert(V_row->dofmap()->cell_dofs(cell_row), col_dofs);
      }

      if (row_has_slaves)
      {
        master_rows.clear();
        expand_masters_for_slave(master_rows, bs_row, *row_master_map,
                                 cell_to_row_slaves->links(cell_row));
        pattern.insert(master_rows, col_dofs);
      }
    };
      ...
      case dolfinx::fem::IntegralType::cell:
        for (int i = 0; i < a.num_integrals(type, 0); ++i)
        {
          ...
          for (std::size_t e = 0; e < cells_row.size(); ++e)
            insert_entity(cells_row[e], cells_col[e]);
        }
        break;
      case dolfinx::fem::IntegralType::exterior_facet:
        for (int i = 0; i < a.num_integrals(type, 0); ++i)
        {
          ...
          std::span<const std::int32_t> facets_row
              = a.domain_arg(type, 0, i, 0);
          std::span<const std::int32_t> facets_col
          ...
          for (std::size_t e = 0; e < facets_row.size(); e += 2)
            insert_entity(facets_row[e], facets_col[e]);
        }
        break;
      default:
        // Matches the limitation of the assembler, which throws on interior
        // facet integrals, so fail here rather than build a pattern that
        // cannot be assembled into.
        throw std::runtime_error(
            "Multi point constraint sparsity patterns are not implemented for "
