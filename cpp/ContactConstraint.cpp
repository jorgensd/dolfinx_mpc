    ...
    std::shared_ptr<dolfinx::fem::Function<PetscScalar>> nh, const double eps2)
{
  ...
  std::vector<std::int32_t> ghost_slave_blocks;
  {
    std::vector<std::int32_t> slave_dofs
        = impl::locate_slave_dofs(V, meshtags, slave_marker);
  ...
  mpc_data mpc_in_cell = impl::compute_block_contributions(
  ...
  std::array<std::size_t, 2> coord_shape;
  {
    ...
    mpc_master_local = impl::compute_master_contributions(
  ...
  mpc_data remote_data;
  {
    ...
    remote_data = impl::compute_master_contributions(
