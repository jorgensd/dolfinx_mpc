...
- **New demos**: `python/demos/demo_mean_value_constraint.py` and `python/demos/demo_facet_integral_constraint.py` show how an affine constraint enforces an integral condition $\int_\Omega u\,\mathrm{d}x = \gamma$ or $\int_\Gamma u\,\mathrm{d}s = \gamma$, as a substitute for a real-space Lagrange multiplier. Both verify against a `basix.ufl.real_element` solve and measure the trade-off: for a cell integral every dof becomes a master, so $K^HAK$ is essentially full and its condition number is inflated by a factor $\sim M$; for a facet integral the master set is only the boundary and the constraint is the cheaper of the two formulations. The facet demo also covers a blocked (Stokes) system with a prescribed flow rate.
